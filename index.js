...
// مسیر برای دسترسی عمومی به فایل‌های آپلود شده
...
async function initDB() {
    try {
        ...
        // Migration های جدید - فقط در صورتی که وجود نداشته باشند ساخته میشوند (بدون حذف داده های قبلی)
        const alterQueries = [
        ...
        for (let q of alterQueries) {
            ...
            } catch (e) { } // Ignore errors if already exists
        }
...
// ========== مسیر آپلود فایل HTTP ==========
app.post('/upload', upload.single('file'), (req, res) => {
    if (!req.file) return res.status(400).send('No file uploaded.');
    // ارسال آدرس و مشخصات فایل به کلاینت
    res.json({ 
...
io.on('connection', (socket) => {
    ...
    socket.on('message', async (data) => {
        const { sessionId, text, tempId, replyToId, file_url, file_name, file_type } = data;
        const isSenderAdmin = socket.data.isAdmin;

        if (!sessionId || (!text && !file_url)) return;

        try {
            const res = await pool.query(
                `INSERT INTO p_messages 
                (session_id, is_admin, text, reply_to_id, reactions, file_url, file_name, file_type) 
                VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id, created_at`,
                [sessionId, isSenderAdmin, text || '', replyToId || null, '{}', file_url || null, file_name || null, file_type || null]
            );
            ...
            const payload = {
                ...
                reply_to: replyData,
                reactions: {},
                file_url, file_name, file_type, is_edited: false
            };
    ...
    socket.on('get_history', async (data) => {
        ...
        try {
            ...
            socket.emit('history_data', res.rows.map(m => ({
                ...
                reactions: m.reactions || {}, file_url: m.file_url, file_name: m.file_name, file_type: m.file_type, is_edited: m.is_edited,
