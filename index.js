...
io.on('connection', (socket) => {
        ...
        try {
            ...
            if (replyToId) {
                const replyRes = await pool.query(
                    'SELECT id, session_id, is_admin, text, created_at FROM p_messages WHERE id = $1',
                    [replyToId]
                );
    ...
    socket.on('add_reaction', async (data) => {
        ...
        try {
            ...
            // فقط ادمین یا صاحب پیام می‌تواند ری‌اکشن بزند
            ...
            // دریافت تمام ری‌اکشن‌های این پیام
    ...
    // === 6. تاریخچه کامل پیام‌ها با ری‌اکشن‌ها ===
    socket.on('get_full_history', async (sessionId) => {
        ...
        try {
            ...
            // دریافت تمام ری‌اکشن‌ها برای این نشست
            const reactionsRes = await pool.query(
                `SELECT 
                    mr.message_id,
                    mr.emoji,
            ...
            // سازماندهی ری‌اکشن‌ها بر اساس message_id
            ...
            // ساخت پیام‌ها با اطلاعات کامل
            const messages = [];
            const messageMap = {};
            
            // اول همه پیام‌ها را ایجاد کن
            ...
            // سپس ریپلای‌ها را وصل کن
            messages.forEach(msg => {
                if (msg.replyToId && messageMap[msg.replyToId]) {
                    const parent = messageMap[msg.replyToId];
                    msg.replyTo = {
                        id: parent.id,
                        text: parent.text,
    ...
    // === 8. مدیریت قطع اتصال و مقاوم‌سازی ===
...
server.listen(PORT, () => {
    console.log(`🚀 Server running on port ${PORT} with full chat features`);
});
